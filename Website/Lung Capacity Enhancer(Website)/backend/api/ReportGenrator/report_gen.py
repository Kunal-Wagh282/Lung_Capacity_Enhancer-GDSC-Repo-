...
def generate_report(data):
    ...
    for i in range(len(data)):
        if i == 0:
            ...
            c.drawString(40, 670, f"Volumen : {data[i]['total_volume']}")
            renderPDF.draw(drawing, c, 100, 400)
        elif i == 1:
            drawing = draw(data, i)

            c.drawString(40, 360, f"Volumen : {data[i]['total_volume']}")
            renderPDF.draw(drawing, c, 100, 90)
            c.showPage()
        else:
            if i % 2 == 0:
                drawing = draw(data, i)
                c.drawString(40, 750, f"Volumen : {data[i]['total_volume']}")
                renderPDF.draw(drawing, c, 100, 480)
            else:
                drawing = draw(data, i)
                c.drawString(40, 440, f"Volumen : {data[i]['total_volume']}")
